...
describe('generate-fit-assets', () => {
  ...
  it('local assets are deterministic and reference the fit theme palette', async () => {
    const fit = catalog.getEventFit('march-madness')
    const plan = buildGenerationPlan(fit, engines)
    ...
    try {
      for (const assetType of LOCAL_ASSET_TYPES) {
        ...
        assert.ok(asset, `missing local asset plan for ${assetType}`)
        ...
        assert.strictEqual(bufOne.length, bufTwo.length, `${assetType} length differs`)
        assert.strictEqual(
          crypto.createHash('sha256').update(bufOne).digest('hex'),
          crypto.createHash('sha256').update(bufTwo).digest('hex'),
          `${assetType} checksum differs`
        )

        const text = bufOne.toString('utf8')
        assert.ok(text.startsWith('<?xml') || text.startsWith('<svg'), `${assetType} is not SVG`)
        assert.ok(text.includes(`data-fit-id="${fit.fitId}"`), `${assetType} missing fit id`)
        assert.ok(text.includes('data-palette='), `${assetType} missing palette attribute`)

        for (const token of profile.palette) {
          assert.ok(
            text.includes(token) || text.includes(`--fit-palette-`),
            `${assetType} does not reference palette token ${token}`
          )
        }
      }
    } finally {
