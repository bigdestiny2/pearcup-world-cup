...
const RENDERER_MAIN = '/lobby-app/index.html'
...
function normalizeRootRendererUrl (url) {
  if (url === '' || url === '/' || url === '//') return RENDERER_MAIN
  if (url[0] === '?') return RENDERER_MAIN + url
  if (url[0] === '#') return RENDERER_MAIN + url
  if (url.startsWith('/?')) return RENDERER_MAIN + url.slice(1)
  if (url.startsWith('//?')) return RENDERER_MAIN + url.slice(2)
  if (url.startsWith('/#')) return RENDERER_MAIN + url.slice(1)
  if (url.startsWith('//#')) return RENDERER_MAIN + url.slice(2)
  ...
  if (base === '' || base === '/' || base === '//') return RENDERER_MAIN + url.slice(plus)
  return url
}
...
main().catch((err) => {
  console.error('[ultimate-sports] fatal', err)
