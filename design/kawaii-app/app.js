...
const PearCupWorkerSim = needModule('PearCupWorkerSim')
const PearCupWorkerClient = needModule('PearCupWorkerClient')
const PearCupTransportSim = needModule('PearCupTransportSim')
const PearCupStorageSim = needModule('PearCupStorageSim')
const PearCupSettlementService = needModule('PearCupSettlementService')
...
function boot () {
  ...
  try {
    ...
    if (joinCode) setTimeout(() => { if (window.PearCupPeerMatch) { setView('games'); PearCupPeerMatch.join(joinCode.toLowerCase()) } }, 300)
  } catch (e) { /* no URL params */ }
  detectLiveRelay()
  // Keep watching for the relay to appear (e.g. kickoff) even if it wasn't there at boot.
  setInterval(() => { if (!isLiveApi()) detectLiveRelay() }, 30000)
