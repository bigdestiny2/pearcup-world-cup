...
contract, and now hard-fails on a dirty worktree unless `--allow-dirty` is passed
for an exploratory run. The live browser/runtime proof still comes from the
post-publish remote friend test.
